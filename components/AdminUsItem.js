  ...
  const handleDeleteUser = async () => {
    try {
      const response = await axios.delete(
        `http://192.168.1.14:8080/admin/deleteUser/${clave_usuario}`
      );
      ...
      <Card style={styles.card}>
        <Card.Content>
          <View style={styles.fieldContainer}>
            ...
            <Text style={styles.fieldPrimaryID}>{clave_usuario}</Text>
            <MaterialCommunityIcons name="tag" size={20} color="#145498" />
            <Text style={styles.fieldPrimaryID}>{privilegio}</Text>
          </View>
          ...
          >
            <View style={styles.modalContainer}>
              <ScrollView contentContainerStyle={styles.scrollContent}>
                ...
                <TextInput
                  style={[styles.input, styles.leftInput]}
                  value={editMode ? updatedEstatus : estatus}
                  label={"Estatus"}
                  onChangeText={setUpdatedEstatus}
                  mode="outlined"
                  activeOutlineColor="#145498"
                  disabled={!editMode}
                />
